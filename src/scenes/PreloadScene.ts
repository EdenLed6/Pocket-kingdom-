...
export class PreloadScene extends Phaser.Scene {
  ...
  private generateUniquePixelArtSet(): void {
    ...
    const unit = (key:string, fw:number, fh:number, frames:number, body:string, team:string): void => mk(key, fw*frames, fh, (ctx) => {
      for (let i=0;i<frames;i++){
        const ox=i*fw; const bob=(i%2)*2;
        rect(ctx, ox + (fw>>1)-16, fh-104-bob, 32, 40, body);
        rect(ctx, ox + (fw>>1)-10, fh-124-bob, 20, 20, '#e6c39e');
        rect(ctx, ox + (fw>>1)-16, fh-94-bob, 32, 6, team);
        rect(ctx, ox + (fw>>1)-14, fh-64, 10, 28+bob, '#4f3b2d');
        rect(ctx, ox + (fw>>1)+4, fh-64+bob, 10, 28, '#4f3b2d');
      }
    });
    ...
    // Units
    unit('worker',192,192,8,'#4b89c7','#f2dc5d'); unit('worker_run',192,192,6,'#4b89c7','#f2dc5d');
    unit('worker_chop',192,192,6,'#4b89c7','#f2dc5d'); unit('worker_mine',192,192,6,'#4b89c7','#f2dc5d');
    unit('worker_build',192,192,3,'#4b89c7','#f2dc5d'); unit('worker_knife',192,192,4,'#4b89c7','#f2dc5d');
    unit('u_archer',192,192,6,'#4f9f72','#f2dc5d'); unit('u_knight',192,192,8,'#8d67ad','#f2dc5d'); unit('u_spearman',320,320,12,'#6b95cd','#f2dc5d');
    unit('u_bandit_archer',192,192,6,'#555','#cf5f5f'); unit('u_bandit_grunt',192,192,8,'#666','#cf5f5f'); unit('u_bandit_raider',320,320,12,'#444','#cf5f5f');
    ...
    unit('deer',128,128,6,'#d8d8d8','#b2b2b2');
    for (let i=1;i<=4;i++) rock(`water_rock_${i}`,'#8d98a8');
  }
